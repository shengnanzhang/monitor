...
readonly DOMAIN="jd.com"
readonly LOGFILE="/var/log/nat-monitor"
#定义的是命令执行的超时时间
readonly TIMESEC="30"
...
{
    result=$(timeout $TIMESEC $COMMAND $DOMAIN)
...
{  
    check_tools
    check_prometheus
    metric="nat_monitor_status"
    local start=$(date +%s%N)
    status=$(monitor_action)
    local end=$(date +%s%N)
    local cost=$[$end-$start]
    content="$content$metric $status\nnat_icmp_cost $cost\n"
    cd /var/lib/node_exporter/textfile && echo -e $content > nat_monitor.prom
