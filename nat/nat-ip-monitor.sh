...
# 未实现的功能4：如果可能，可以使用dig www.baidu.com +short @nat-master来直接检查master上ip生效的情况，避免延时生效导致大规模故障的发生
...
readonly DOMAIN="120.52.148.118"

#定义的是命令执行的超时时间
readonly TIMESEC="30"

readonly COMMAND="fping"

#将输出结果默认赋值
result="-1"
natip_monitor_status="-1"
...
function check_prometheus
{
    ...
    cd /var/lib/node_exporter/textfile && touch natip_monitor.prom && chmod 755 natip_monitor.prom
}
...
{
    result=$(timeout $TIMESEC $COMMAND $DOMAIN |grep "alive")
    if [ -z "$result" ]; then
        echo 1
    else
        echo 0
...
{
    if [ "$result" == "$DOMAIN is alive" ]; then
        cd /var/lib/node_exporter/textfile && echo "natip_monitor_status 0" >  natip_monitor.prom
    else
        cd /var/lib/node_exporter/textfile && echo "natip_monitor_status 1" >  natip_monitor.prom
    fi
}
...
{  
    check_tools
    check_prometheus
    metric="natip_monitor_status"
    local start=$(date +%s%N)
    status=$(monitor_action)
    local end=$(date +%s%N)
    local cost=$[$end-$start]
    content="$content$metric $status\nnat_icmp_cost $cost\n"
    cd /var/lib/node_exporter/textfile && echo -e $content > natip_monitor.prom
